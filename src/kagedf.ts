...
export function dfDrawFont(
	...
	{ a1, a2, a3, x1, y1, x2, y2, x3, y3, x4, y4, tateAdjustment, mageAdjustment }: Stroke) {
			...
			case 3: {
				if (a3 % 1000 === 5) {
					...
					if ((x2 < x3 && tx3 - tx2 > 0) || (x2 > x3 && tx2 - tx3 > 0)) { // for closer position
						cdDrawLine(kage, polygons, tx2, ty2, tx3, ty3, 6 + mageAdjustment * 1000, 5); // bolder by force
					}
				} else {
					const [dx1, dy1] = (x1 === x2 && y1 === y2)
						? [0, kage.kMage] // ?????
					...
					cdDrawLine(kage, polygons, x1, y1, tx1, ty1, a2, 1);
					cdDrawCurve(kage, polygons, tx1, ty1, x2, y2, tx2, ty2, 1 + tateAdjustment * 10000, 1 + mageAdjustment * 1000);
					cdDrawLine(kage, polygons, tx2, ty2, x3, y3, 6 + mageAdjustment * 1000, a3); // bolder by force
				}
				break;
			}
			case 12: {
