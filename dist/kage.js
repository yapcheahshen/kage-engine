	...
	if (kage.kShotai == kage.kMincho) {
		...
		if (sx1 == sx2 && sy1 == sy2) {
			...
			if (kage.kUseCurve) {
				...
				(0, _curve.get_candidate)(kage2, curve, a1, a2, x1, y1, sx1, sy1, x2, y2, opt3, opt4);

				var dcl12_34 = new Array(2);
				var dcr12_34 = new Array(2);
				var dpl12_34 = new Array(2);
				var dpr12_34 = new Array(2);
				(0, _curve.divide_curve)(kage2, x1, y1, sx1, sy1, x2, y2, curve[0], dcl12_34, dpl12_34);
				(0, _curve.divide_curve)(kage2, x1, y1, sx1, sy1, x2, y2, curve[1], dcr12_34, dpr12_34);

				var ncl1 = new Array(7);
				var ncl2 = new Array(7);
				(0, _curve.find_offcurve)(kage2, dcl12_34[0], dpl12_34[0][2], dpl12_34[0][3], ncl1);
				(0, _curve.find_offcurve)(kage2, dcl12_34[1], dpl12_34[1][2], dpl12_34[1][3], ncl2);
		...
		if (a2 == 1 || a2 == 8 || a2 == 15) {
			...
			if (sx2 == x2) {
				...
				if (kage.kUseCurve) {
					...
					poly.push(x2 + Math.cos(rad) * kMinWidthT2 * 0.9 * v + Math.sin(rad) * kMinWidthT2 * 0.9 * v, y2 + Math.sin(rad) * kMinWidthT2 * 0.9 * v - Math.cos(rad) * kMinWidthT2 * 0.9 * v, 1);
					poly.push(x2 + Math.cos(rad) * kMinWidthT2 * v, y2 + Math.sin(rad) * kMinWidthT2 * v);
					poly.push(x2 + Math.cos(rad) * kMinWidthT2 * 0.9 * v - Math.sin(rad) * kMinWidthT2 * 0.9 * v, y2 + Math.sin(rad) * kMinWidthT2 * 0.9 * v + Math.cos(rad) * kMinWidthT2 * 0.9 * v, 1);
					poly.push(x2 - Math.sin(rad) * kMinWidthT2 * v, y2 + Math.cos(rad) * kMinWidthT2 * v);
				} else {
					poly.push(x2 + Math.sin(rad) * kMinWidthT2 * v, y2 - Math.cos(rad) * kMinWidthT2 * v);
					poly.push(x2 + Math.cos(rad) * kMinWidthT2 * 0.7 * v + Math.sin(rad) * kMinWidthT2 * 0.7 * v, y2 + Math.sin(rad) * kMinWidthT2 * 0.7 * v - Math.cos(rad) * kMinWidthT2 * 0.7 * v);
					poly.push(x2 + Math.cos(rad) * kMinWidthT2 * v, y2 + Math.sin(rad) * kMinWidthT2 * v);
					poly.push(x2 + Math.cos(rad) * kMinWidthT2 * 0.7 * v - Math.sin(rad) * kMinWidthT2 * 0.7 * v, y2 + Math.sin(rad) * kMinWidthT2 * 0.7 * v + Math.cos(rad) * kMinWidthT2 * 0.7 * v);
					poly.push(x2 - Math.sin(rad) * kMinWidthT2 * v, y2 + Math.cos(rad) * kMinWidthT2 * v);
				}
	...
	if (kage.kShotai == kage.kMincho) {
		...
		if (x1 == x2) {
			...
			if (Math.abs(y2 - y1) < Math.abs(x2 - x1) && a1 != 6 && a2 != 6 && !(x1 > x2)) {
				...
				switch (a1) {
					case 0:
						poly.set(0, x1 + Math.sin(rad) * kMinWidthT * v + kage.kMinWidthY * Math.cos(rad) * 0.5 * v, y1 - Math.cos(rad) * kMinWidthT * v + kage.kMinWidthY * Math.sin(rad) * 0.5 * v);
						poly.set(3, x1 - Math.sin(rad) * kMinWidthT * v - kage.kMinWidthY * Math.cos(rad) * 0.5 * v, y1 + Math.cos(rad) * kMinWidthT * v - kage.kMinWidthY * Math.sin(rad) * 0.5 * v);
						break;
					case 1:
					case 6:
						...
						break;
					case 12:
						poly.set(0, x1 + Math.sin(rad) * kMinWidthT * v - kage.kMinWidthY * Math.cos(rad) * v, y1 - Math.cos(rad) * kMinWidthT * v - kage.kMinWidthY * Math.sin(rad) * v);
						poly.set(3, x1 - Math.sin(rad) * kMinWidthT * v - (kMinWidthT + kage.kMinWidthY) * Math.cos(rad) * v, y1 + Math.cos(rad) * kMinWidthT * v - (kMinWidthT + kage.kMinWidthY) * Math.sin(rad) * v);
						break;
					case 22:
				...
				switch (a2) {
					case 0:
						if (a1 == 6) {
							...
							poly.set(1, x2 + Math.sin(rad) * kMinWidthT * v - kMinWidthT * 0.5 * Math.cos(rad) * v, y2 - Math.cos(rad) * kMinWidthT * v - kMinWidthT * 0.5 * Math.sin(rad) * v);
							poly.set(2, x2 - Math.sin(rad) * kMinWidthT * v + kMinWidthT * 0.5 * Math.cos(rad) * v, y2 + Math.cos(rad) * kMinWidthT * v + kMinWidthT * 0.5 * Math.sin(rad) * v);
						}
						break;
					case 1: // is needed?
					case 5:
						...
						break;
					case 13:
						poly.set(1, x2 + Math.sin(rad) * kMinWidthT * v + kage.kAdjustKakatoL[opt2] * Math.cos(rad) * v, y2 - Math.cos(rad) * kMinWidthT * v + kage.kAdjustKakatoL[opt2] * Math.sin(rad) * v);
						poly.set(2, x2 - Math.sin(rad) * kMinWidthT * v + (kage.kAdjustKakatoL[opt2] + kMinWidthT) * Math.cos(rad) * v, y2 + Math.cos(rad) * kMinWidthT * v + (kage.kAdjustKakatoL[opt2] + kMinWidthT) * Math.sin(rad) * v);
						break;
					case 23:
						poly.set(1, x2 + Math.sin(rad) * kMinWidthT * v + kage.kAdjustKakatoR[opt2] * Math.cos(rad) * v, y2 - Math.cos(rad) * kMinWidthT * v + kage.kAdjustKakatoR[opt2] * Math.sin(rad) * v);
						poly.set(2, x2 - Math.sin(rad) * kMinWidthT * v + (kage.kAdjustKakatoR[opt2] + kMinWidthT) * Math.cos(rad) * v, y2 + Math.cos(rad) * kMinWidthT * v + (kage.kAdjustKakatoR[opt2] + kMinWidthT) * Math.sin(rad) * v);
						break;
					case 24:
		...
		switch (a1 % 100) {// ... no need to divide
			...
			case 2:
				...
				if (a3 % 100 === 4) {
					...
					(0, _kagecd.cdDrawCurve)(kage, polygons, x1, y1, x2, y2, tx1, ty1, a2, 1);
					(0, _kagecd.cdDrawCurve)(kage, polygons, tx1, ty1, x3, y3, x3 - kage.kMage, y3, 1, a3 + 10);
				} else if (a3 == 5) {
			...
			case 3:
				if (a3 % 1000 === 5) {
					...
					(0, _kagecd.cdDrawLine)(kage, polygons, x1, y1, tx1, ty1, a2, 1);
					(0, _kagecd.cdDrawCurve)(kage, polygons, tx1, ty1, x2, y2, tx2, ty2, 1 + (a2 - a2 % 1000) * 10, 1 + (a3 - a3 % 1000));
					...
					(0, _kagecd.cdDrawLine)(kage, polygons, x1, y1, tx1, ty1, a2, 1);
					(0, _kagecd.cdDrawCurve)(kage, polygons, tx1, ty1, x2, y2, tx2, ty2, 1 + (a2 - a2 % 1000) * 10, 1 + (a3 - a3 % 1000));
					(0, _kagecd.cdDrawLine)(kage, polygons, tx2, ty2, x3, y3, 6 + (a3 - a3 % 1000), a3); // bolder by force
				}
				break;
			case 12:
			...
			case 4:
				...
				if (a3 == 5) {
					...
					(0, _kagecd.cdDrawLine)(kage, polygons, x1, y1, tx1, ty1, a2, 1);
					(0, _kagecd.cdDrawCurve)(kage, polygons, tx1, ty1, x2, y2, tx2, ty2, 1, 1);
					if (tx3 - tx2 > 0) {
						// for closer position
						(0, _kagecd.cdDrawLine)(kage, polygons, tx2, ty2, tx3, ty3, 6, 5); // bolder by force
					}
				} else {
					if (x1 == x2) {
						if (y1 < y2) {
					...
					(0, _kagecd.cdDrawLine)(kage, polygons, x1, y1, tx1, ty1, a2, 1);
					(0, _kagecd.cdDrawCurve)(kage, polygons, tx1, ty1, x2, y2, tx2, ty2, 1, 1);
					(0, _kagecd.cdDrawLine)(kage, polygons, tx2, ty2, x3, y3, 6, a3); // bolder by force
				}
				break;
			case 6:
				if (a3 % 100 === 4) {
					...
					(0, _kagecd.cdDrawCurve)(kage, polygons, tx1, ty1, x4, y4, x4 - kage.kMage, y4, 1, a3 + 10);
				} else if (a3 == 5) {
		...
		switch (a1 % 100) {
			...
			case 1:
				if (a3 == 4) {
					...
					(0, _kagecd.cdDrawLine)(kage, polygons, x1, y1, tx1, ty1, a2, 1);
					(0, _kagecd.cdDrawCurve)(kage, polygons, tx1, ty1, x2, y2, x2 - kage.kMage * 2, y2 - kage.kMage * 0.5, 1, 0);
				} else {
					(0, _kagecd.cdDrawLine)(kage, polygons, x1, y1, x2, y2, a2, a3);
				}
				break;
			...
			case 12:
				if (a3 == 4) {
					...
					(0, _kagecd.cdDrawCurve)(kage, polygons, x1, y1, x2, y2, tx1, ty1, a2, 1);
					(0, _kagecd.cdDrawCurve)(kage, polygons, tx1, ty1, x3, y3, x3 - kage.kMage * 2, y3 - kage.kMage * 0.5, 1, 0);
				} else if (a3 == 5) {
					tx1 = x3 + kage.kMage;
					ty1 = y3;
					...
					ty2 = y3 - kage.kMage * 2;
					(0, _kagecd.cdDrawCurve)(kage, polygons, x1, y1, x2, y2, x3, y3, a2, 1);
					(0, _kagecd.cdDrawCurve)(kage, polygons, x3, y3, tx1, ty1, tx2, ty2, 1, 0);
				} else {
					(0, _kagecd.cdDrawCurve)(kage, polygons, x1, y1, x2, y2, x3, y3, a2, a3);
				}
				break;
			case 3:
				if (a3 == 5) {
					...
					(0, _kagecd.cdDrawLine)(kage, polygons, x1, y1, tx1, ty1, a2, 1);
					(0, _kagecd.cdDrawCurve)(kage, polygons, tx1, ty1, x2, y2, tx2, ty2, 1, 1);
					(0, _kagecd.cdDrawLine)(kage, polygons, tx2, ty2, tx3, ty3, 1, 1);
					(0, _kagecd.cdDrawCurve)(kage, polygons, tx3, ty3, x3, y3, tx4, ty4, 1, 0);
				} else {
					if (x1 == x2) {
						if (y1 < y2) {
							v = 1;
					...
					(0, _kagecd.cdDrawLine)(kage, polygons, x1, y1, tx1, ty1, a2, 1);
					(0, _kagecd.cdDrawCurve)(kage, polygons, tx1, ty1, x2, y2, tx2, ty2, 1, 1);
					(0, _kagecd.cdDrawLine)(kage, polygons, tx2, ty2, x3, y3, 1, a3);
				}
				break;
			case 6:
				if (a3 == 5) {
