...
var Kage = (function () {
    ...
    function cdDrawCurveU(font, polygons, x1, y1, sx1, sy1, sx2, sy2, x2, y2, ta1, ta2, opt1, haneAdjustment, opt3, opt4) {
        ...
        var _a = (x1 === sx1 && y1 === sy1)
            ? [0, delta] // ?????
            : normalize([x1 - sx1, y1 - sy1], delta), dx1 = _a[0], dy1 = _a[1];
        x1 += dx1;
        y1 += dy1;
        switch (a2 % 100) {
            case 0:
        ...
        var _b = (sx2 === x2 && sy2 === y2)
            ? [0, -delta] // ?????
            : normalize([x2 - sx2, y2 - sy2], delta), dx2 = _b[0], dy2 = _b[1];
        x2 += dx2;
        y2 += dy2;
        var isQuadratic = sx1 === sx2 && sy1 === sy2;
        // ---------------------------------------------------------------
        if (isQuadratic && font.kUseCurve) {
            ...
            if (hypot(x2 - x1, y2 - y1) < 50) {
                hosomi_1 += 0.4 * (1 - hypot(x2 - x1, y2 - y1) / 50);
            }
                ...
                if (isQuadratic) {
                    ...
                    deltad
                        = a1 === 7 && a2 === 0 // L2RD: fatten
                            ? Math.pow(t, hosomi_1) * font.kL2RDfatten
                            : a1 === 7
                                ? Math.pow(t, hosomi_1)
                                : a2 === 7
                                    ? Math.pow((1 - t), hosomi_1)
                                    : opt3 > 0 || opt4 > 0
                    ...
                    deltad
                        = a1 === 7 && a2 === 0 // L2RD: fatten
                            ? Math.pow(t, hosomi_1) * font.kL2RDfatten
                            : a1 === 7
                                ? Math.pow((Math.pow(t, hosomi_1)), 0.7) // make fatten
                                : a2 === 7
                                    ? Math.pow((1 - t), hosomi_1)
                                    : 1;
                }
        ...
        if (a1 === 12) {
            var _m = (x1 === x2)
                ...
                : (sx1 === x1)
                    ...
                    : normalize([sy1 - y1, -(sx1 - x1)]), dx = _m[0], dy = _m[1];
            var poly = new Polygon([
                { x: -kMinWidthT, y: 0 },
                { x: +kMinWidthT, y: 0 },
        ...
        if (a1 === 0) {
            if (y1 <= y2) { // from up to bottom
                ...
                var _o = (x1 === sx1)
                    ? [1, 0] // ?????
                    : normalize([sy1 - y1, -(sx1 - x1)]), XX = _o[0], XY = _o[1];
                var poly = new Polygon([
                    { x: -kMinWidthT, y: 1 },
                    { x: +kMinWidthT, y: 0 },
                ...
                var _p = (x1 === sx1)
                    ? [1, 0] // ?????
                    : normalize([sy1 - y1, -(sx1 - x1)]), XX = _p[0], XY = _p[1];
                var poly = new Polygon([
                    { x: -kMinWidthT, y: 0 },
                    { x: +kMinWidthT, y: 0 },
        ...
        if (a2 === 9 || (a1 === 7 && a2 === 0)) { // Math.sinnyu & L2RD Harai ... no need for a2=9
            ...
            var _r = (sy2 === y2)
                ...
                : (sx2 === x2)
                    ...
                    : normalize([-(y2 - sy2), x2 - sx2], 1), dx = _r[0], dy = _r[1];
            var poly = new Polygon([
        ...
        if (a2 === 14) { // jump to left, allways go left
            var poly = new Polygon([
                { x: 0, y: 0 },
                { x: 0, y: -kMinWidthT },
                { x: -font.kWidth * 4 * Math.min(1 - haneAdjustment / 10, Math.pow((kMinWidthT / font.kMinWidthT), 3)), y: -kMinWidthT },
                { x: -font.kWidth * 4 * Math.min(1 - haneAdjustment / 10, Math.pow((kMinWidthT / font.kMinWidthT), 3)), y: -kMinWidthT * 0.5 },
            ]);
            // poly.reverse();
            poly.translate(x2, y2);
            polygons.push(poly);
    ...
    function cdDrawLine(font, polygons, tx1, ty1, tx2, ty2, ta1, ta2, opt1, urokoAdjustment, kakatoAdjustment, mageAdjustment) {
        ...
        if (x1 === x2 || y1 !== y2 && (x1 > x2 || Math.abs(y2 - y1) >= Math.abs(x2 - x1) || a1 === 6 || a2 === 6)) {
            ...
            // for others, use x-axis
            // ASAI KAUDO
            var _c = (y1 === y2) ? [1, 0] : normalize([x2 - x1, y2 - y1]), cosrad = _c[0], sinrad = _c[1];
            // always same
            var poly = new Polygon([
        ...
        switch (a1 % 100) { // ... no need to divide
            ...
            case 2: {
                ...
                if (a3_100 === 4) {
                    ...
                    var tx1 = x3 + dx1;
                    var ty1 = y3 + dy1;
                    cdDrawCurve(font, polygons, x1, y1, x2, y2, tx1, ty1, a2_100 + kirikuchiAdjustment * 100, 1, tateAdjustment, 0, opt3, 0);
    ...
    ***************************************************************************** */
    ...
    function cdDrawCurveU$1(font, polygons, x1, y1, sx1, sy1, sx2, sy2, x2, y2, ta1, ta2) {
        ...
        if (a1 % 10 === 2) {
            var _a = (x1 === sx1 && y1 === sy1)
                ? [0, font.kWidth] // ?????
                : normalize([x1 - sx1, y1 - sy1], font.kWidth), dx1 = _a[0], dy1 = _a[1];
            x1 += dx1;
            y1 += dy1;
        }
        else if (a1 % 10 === 3) {
            var _b = (x1 === sx1 && y1 === sy1)
                ? [0, font.kWidth * font.kKakato] // ?????
                : normalize([x1 - sx1, y1 - sy1], font.kWidth * font.kKakato), dx1 = _b[0], dy1 = _b[1];
            x1 += dx1;
            y1 += dy1;
        }
        if (a2 % 10 === 2) {
            var _c = (sx2 === x2 && sy2 === y2)
                ? [0, -font.kWidth] // ?????
                : normalize([x2 - sx2, y2 - sy2], font.kWidth), dx2 = _c[0], dy2 = _c[1];
            x2 += dx2;
            y2 += dy2;
        }
        else if (a2 % 10 === 3) {
            var _d = (sx2 === x2 && sy2 === y2)
                ? [0, -font.kWidth * font.kKakato] // ?????
                : normalize([x2 - sx2, y2 - sy2], font.kWidth * font.kKakato), dx2 = _d[0], dy2 = _d[1];
            x2 += dx2;
            y2 += dy2;
        }
        var _e = generateFattenCurve(x1, y1, sx1, sy1, sx2, sy2, x2, y2, font.kRate, function () { return font.kWidth; }, function (_a, mag) {
            var x = _a[0], y = _a[1];
            return (round(x) === 0 && round(y) === 0)
                ? [-mag, 0] // ?????
                : normalize([x, y], mag);
        }), left = _e.left, right = _e.right;
        var poly = new Polygon();
        var poly2 = new Polygon();
        // save to polygon
        for (var _i = 0, left_1 = left; _i < left_1.length; _i++) {
            var _f = left_1[_i], x = _f[0], y = _f[1];
            poly.push(x, y);
        }
        for (var _g = 0, right_1 = right; _g < right_1.length; _g++) {
            var _h = right_1[_g], x = _h[0], y = _h[1];
            poly2.push(x, y);
        }
        poly2.reverse();
        poly.concat(poly2);
    ...
    function cdDrawLine$1(font, polygons, tx1, ty1, tx2, ty2, ta1, ta2) {
        ...
        var _a = (x1 === x2 && y1 === y2)
            ? [0, font.kWidth] // ?????
            : normalize([x2 - x1, y2 - y1], font.kWidth), dx = _a[0], dy = _a[1];
        if (a1 % 10 === 2) {
            x1 -= dx;
            y1 -= dy;
        }
